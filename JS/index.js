...
function addToDo(event) {
    if (event.key == "Enter" && newElement.value != "") {
...
function toMiddle(sigma) {
    ...
    // adding part
    doingList.innerHTML += "<li id='li2" + doingAmmount + "'> <input type='checkbox' onclick='toFinished(this)' id='checkbox2" + doingAmmount + "'  class='check'>" + document.getElementById("li" + value).innerText + "</li>";
    // removal part
...
function toFinished(sigma) {
    ...
    // adding part
    doneList.innerHTML += "<li id='li3" + finishedAmmount + "'> <button id='button" + finishedAmmount + "' onclick='itIsOverJohnny(this)' title='Click to remove task from finished' class='but'>X</button>" + document.getElementById("li2" + value).innerText + "</li>";
    // removal part
